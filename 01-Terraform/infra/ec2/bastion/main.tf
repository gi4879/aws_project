resource "aws_instance" "aws06_bastion" {
    ami = data.aws_ami.ubuntu.image_id
    instance_type = "t2.micro"
    key_name = "aws06-key"
    vpc_security_group_ids = [aws_security_group.aws06_ssh_sg.id]
    subnet_id = data.terraform_remote_state.aws06_vpc.outputs.public_subnet2a
